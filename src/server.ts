...
import { z } from 'zod'
import type { LlmClient } from './lib/llm/openaiCompatible.js'
...
function makeLlmClient(): LlmClient {
  ...
  let apiKey = (llmKeyRaw || qiniuKeyRaw || '') as string
  if (!apiKey) {
...
function checkAccessToken(req: express.Request): boolean {
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb: multer.FileFilterCallback) => {
    if (isAllowedUploadFilename(file.originalname)) return cb(null, true)
    return cb(
      new Error('仅支持上传：.docx / .pdf / .pptx / .pages（旧版 .ppt 请先另存为 .pptx）')
    )
  }
})
...
type SuggestAudienceMode = 'market_only' | 'gov_only' | 'both'

function suggestAudienceMode(audiences: Array<z.infer<typeof QuestionAudienceEnum>>): SuggestAudienceMode {
  const m = audiences.includes('market_investor')
  const g = audiences.includes('gov_guidance_fund')
  if (m && g) return 'both'
  if (g) return 'gov_only'
  return 'market_only'
}

function looksMedTechRelated(industry: string, product: string, definition: string): boolean {
...
function suggestQuestionsSystemMessage(mode: SuggestAudienceMode): string {
  const base =
    '你只输出最终内容：每行一个完整问句。不要序号、不要分组标题（如A类B类/1)/2)）、不要括号说明、不要思考过程。'
  const cnStartup =
    ' 必须包含「××领域创业公司有哪些」「××赛道创业公司有哪些」类基础句式（××用用户行业词）；并包含「国内」「创业公司」「有哪些」组合行。不得用「头部玩家」替代「创业公司/初创」字面。'
  if (mode === 'both') {
    return (
      base +
      cnStartup +
...
function suggestKeywordSource(industry: string, product: string, definition: string): string {
...
function countLiteralDomesticStartupWhichLines(questions: string[]): number {
  return questions.filter((q) => q.includes('国内') && q.includes('创业公司') && q.includes('有哪些')).length
}

function pickTopicPhrase(industry: string, product: string, definition: string): string {
  const a = industry.trim()
  const b = product.trim()
  const c = definition.trim()
  if (a) return a
  if (b) return b
  if (c) return c.length > 24 ? `${c.slice(0, 24)}…` : c
  return '该领域'
}
 ...
 */
function ensureLiteralDomesticStartupQuestions(
  questions: string[],
  industry: string,
  product: string,
  definition: string
  ...
  if (need <= 0) return questions
  const injected: string[] = []
  if (need >= 1) injected.push(`国内${topic}创业公司有哪些`)
  if (need >= 2) injected.push(`国内做${topic}的创业公司有哪些`)
...
function countDomainOrTrackStartupWhich(questions: string[]): number {
  return questions.filter(
    (q) =>
      (q.includes('创业公司') || q.includes('初创公司')) &&
      q.includes('有哪些') &&
      (q.includes('领域') || q.includes('赛道'))
  ).length
}

function ensureBasicStartupListingQuestions(
  questions: string[],
  industry: string,
  product: string,
  definition: string
  ...
  const hasStartupWhich = questions.some(
    (q) =>
      q.includes('创业公司有哪些') ||
      q.includes('初创公司有哪些') ||
...
function hasChinaChuangListLine(questions: string[]): boolean {
  return questions.some((q) => q.includes('中国') && q.includes('初创') && q.includes('清单'))
}

function ensureChinaChuangListQuestion(
  questions: string[],
  industry: string,
  product: string,
  definition: string
): string[] {
  if (hasChinaChuangListLine(questions)) return questions
  const topic = pickTopicPhrase(industry, product, definition)
...
type ScoreBreakdownItem = { key: string; score: number; max: number; detail?: string }

function extractIndustryKeywords(industry: string, product: string, definition: string): string[] {
...
function countTripleComboLines(questions: string[]): number {
  return questions.filter(
    (q) =>
      (q.includes('国内') || q.includes('中国')) &&
      (q.includes('创业公司') || q.includes('初创')) &&
      (q.includes('有哪些') || q.includes('清单'))
  ).length
}
...
const DD_INTERVIEW_RE =
  /贵公司|贵司|你们公司|毛利率|净利率|多少亿|IRR|EBITDA|留存率|LTV|CAC|Payback|本轮融资|估值区间|尽职调查|投决|投委会|护城河是什么|壁垒是什么|销售周期多长|客单价|续约率|回款周期|ARR|MRR|GMV|烧钱| runway|烧钱率/i

function countDdInterviewLines(questions: string[], companyName: string): number {
  const co = companyName.trim()
  return questions.filter((q) => {
    if (DD_INTERVIEW_RE.test(q)) return true
...
function scoreQuestionSet(
  questions: string[],
  mode: SuggestAudienceMode,
  industry: string,
  product: string,
  definition: string,
  companyName: string
): { score: number; breakdown: ScoreBreakdownItem[]; hints: string[] } {
  ...
  total += sLit
  if (lit < 2) hints.push('增加至少两条「国内…创业公司有哪些」式问句')
  ...
  total += sDomain
  if (domainN < 2) hints.push('必须包含「××领域创业公司有哪些」「××赛道创业公司有哪些」类基础搜索问句')
  ...
  total += sChina
  if (!sChina) hints.push('补一条「中国…初创…清单」式问句')
  ...
  total += sTriple
  if (triple < 5) hints.push('增加「国内/中国+创业公司或初创+有哪些或清单」组合问句')
  ...
  const sFreq = Math.min(
    10,
    Math.round(5 * Math.min(1, cnLines / 6) + 5 * Math.min(1, suLines / 6))
  )
  breakdown.push({
    key: 'cnStartupSpread',
    score: sFreq,
  ...
  total += sFreq
  if (cnLines < 6 || suLines < 6) hints.push('让更多问句分别出现「国内/中国」与「创业公司/初创」')
  ...
  if (n > 0 && keywords.length > 0) {
    ...
    if (cov < 0.55) hints.push('提高问句里对用户行业/产品关键词的覆盖')
  } else {
    ...
    if (keywords.length === 0) hints.push('补充「行业/产品/产品定义」以便问法更贴领域')
  }
  total += sInd
  ...
  if (mode === 'both') {
    ...
    let sGov = 5
    if (r >= 0.22 && r <= 0.58) sGov = 18
    else if (r >= 0.12 && r < 0.7) sGov = 12
    breakdown.push({
      key: 'govAudienceMix',
      score: sGov,
      max: 18,
    ...
    total += sGov
    if (r < 0.2 || r > 0.6) hints.push('双选时增加或减少带园区/返投/国资/引导基金等政府检索语境的问句')
  } else {
  ...
  total += sTrig
  if (trigRatio < 0.18) hints.push('增加对标、替代方案、推荐清单、谁在做等触发式问句')
  ...
  total += sSearchLike
  if (ddLines > 0) hints.push('删掉「毛利率/估值/壁垒/供应商/客户画像」等访谈式问句，改成「有哪些公司/怎么选/对标谁/清单」类搜索问法')

  let sCount = 0
  if (n >= 50 && n <= 80) sCount = 8
  else if (n >= 40 && n <= 95) sCount = 5
  else if (n >= 20) sCount = 2
  breakdown.push({ key: 'countBand', score: sCount, max: 8, detail: `共 ${n} 条（目标约50–80）` })
  total += sCount
  if (n < 45 || n > 85) hints.push('调整生成条数到约50–80条')
  ...
  if (hasCo) {
    ...
    total += sName
    if (nameHits < 10) hints.push('在更多行业检索问句中自然带入公司名')
  } else {
...
function parseQuestionsFromLlmText(text: string): string[] {
  return text
    .split(/\n+/)
...
function minimalFallbackQuestions(
  industry: string,
  product: string,
  definition: string,
  companyName: string
...
function buildSuggestQuestionsUserPrompt(
  mode: SuggestAudienceMode,
  medTechExtra: boolean,
  ...
  companyName: string,
  competitors: string,
  audiences: Array<z.infer<typeof QuestionAudienceEnum>>,
  retryHint?: string
): string {
  const { intro, constraints, quotas, closingLines } = buildSuggestQuestionsPromptParts(
    mode,
    medTechExtra,
    hasCompanyName,
    industry,
...
function buildSuggestQuestionsPromptParts(
  mode: SuggestAudienceMode,
  medTechExtra: boolean,
  hasCompanyName: boolean,
  industry: string,
  ...
  if (hasCompanyName) {
    namingRules.push(
      `- 用户提供了公司名：请至少 12 条问句在行业检索语境下显式包含公司名（例如“X公司对标谁/属于哪一类/与哪些公司同类/在清单里怎么找/和哪些公司一起被列为代表玩家”），但不要每条都硬塞。`,
      `- 至少 6 条包含公司名的问句必须带“对标/替代/同类/代表公司/推荐/清单/谁在做”等触发词之一，避免只是把公司名硬插进融资盘点句式`
    )
  } else {
...
app.post('/api/suggest_questions', async (req, res) => {
  try {
    ...
    let bestScore = -1
    let bestBreakdown: ScoreBreakdownItem[] = []
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      attempts = attempt + 1
      const retryHint =
        attempt > 0 && lastHints.length
          ? `上一轮质量分 ${lastScore}（满分100）。请按下列方向改写或增补问句：${lastHints.join('；')}`
          : undefined

      const userPrompt = buildSuggestQuestionsUserPrompt(
        mode,
        medTechExtra,
        hasCompanyName,
        industry,
...
app.post('/api/jobs', upload.array('uploadFiles', 10), async (req, res) => {
  try {
    ...
    const pastedUnknown = JSON.parse(pastedRaw) as unknown
    const pastedBlocks = PastedBlocksSchema.parse(pastedUnknown)

    const uploadFiles = ((req.files ?? []) as Express.Multer.File[]).map((f) => ({
    ...
    // Run async
    void (async () => {
      const update = (status: JobStatus, message: string, zip?: Buffer) => {
      ...
      try {
        ...
        if (next) {
          ...
          next.fileContents = out.contentFiles.reduce<Record<string, string>>((acc, f) => {
...
server.on('error', (err) => {
  ...
  if (anyErr?.code === 'EADDRINUSE') {
    // eslint-disable-next-line no-console
    console.error(
      `[geo-agent-web] Port ${port} is already in use. ` +
