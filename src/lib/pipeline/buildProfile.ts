import { z } from 'zod'
import type { EntityProfile, Source } from '../types.js'
...
function buildSourceIndex(sources: Source[]): string {
  const blocks: string[] = []
  for (const s of sources) {
    const head = `[${s.id}] type=${s.type}${s.emphasized ? ' emphasized=true' : ''}${
      s.sourceUrl ? ` url=${s.sourceUrl}` : ''
    }${s.filename ? ` file=${s.filename}` : ''}${s.title ? ` title=${s.title}` : ''}`
