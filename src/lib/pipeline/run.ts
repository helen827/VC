...
export const PastedBlocksSchema = z.array(
  z.object({
...
export async function runGeneratePipeline(input: {
  ...
  const msg = (m: string) => input.onMessage?.(m)
  ...
  const staticGuide: GeneratedFile = {
...
export type SopPlanItem = {
  platform: 'website' | 'wechat' | 'zhihu' | 'eval'
  title: string
  path: string
}

export type SopPlanSection = {
  title: string
  instructions: string[]
  items: SopPlanItem[]
}
...
export type SopPlan = {
  companyName: string
  summary: string[]
  sections: SopPlanSection[]
}

function buildSopPlan(input: { companyName: string; contentFiles: GeneratedFile[] }): SopPlan {
  const byPath = new Set(input.contentFiles.map((f) => f.path))
  const find = (p: string) => (byPath.has(p) ? p : null)
  ...
  const sections: SopPlanSection[] = []

  const websiteItems: SopPlanItem[] = []
  if (websiteAbout) websiteItems.push({ platform: 'website', title: '官网 About / 公司介绍页', path: websiteAbout })
  if (websiteFaq) websiteItems.push({ platform: 'website', title: '官网 FAQ（覆盖问法族）', path: websiteFaq })
  sections.push({
    title: '官网（优先级：最高）',
    instructions: [
  ...
  const wechatItems: SopPlanItem[] = []
  if (wechatLong) wechatItems.push({ platform: 'wechat', title: '公众号权威长文（信息密度高，便于引用）', path: wechatLong })
  sections.push({
    title: '公众号（优先级：高）',
  ...
  const zhihuItems: SopPlanItem[] = []
  if (zhihuAnswers) zhihuItems.push({ platform: 'zhihu', title: '知乎问答合集（拆分为多条发布）', path: zhihuAnswers })
  sections.push({
    title: '知乎（优先级：高）',
...
function sopPlanToText(plan: SopPlan): string {
  const lines: string[] = []
  ...
  for (const sec of plan.sections) {
    ...
    for (const i of sec.instructions) lines.push(`- ${i}`)
    for (const it of sec.items) lines.push(`- ${it.title}: ${it.path}`)
