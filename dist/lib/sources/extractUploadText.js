...
function extractATextFromSlideXml(xml) {
    const parts = [];
    const re = /<a:t[^>]*>([^<]*)<\/a:t>/g;
    let m;
...
export async function extractPptxText(buf) {
...
export async function extractPagesText(buf) {
    const zip = await JSZip.loadAsync(buf);
    ...
    throw new Error('无法从 Pages 文件中提取文本（常见新格式不含预览 PDF）。请在 Pages 中「文件 → 导出为 → PDF」或「Word」后再上传。');
}
export async function extractUploadText(filename, buf) {
